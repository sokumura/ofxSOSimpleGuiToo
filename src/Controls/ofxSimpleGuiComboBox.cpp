...
//	if(m_hasFocus && m_mouseMovedSinceClick) {
		releaseEventStealingFocus();
//	}else if (y < height) releaseEventStealingFocus();
}
...
#define kSGCBTextPaddingX    6
#define kSGCBTextPaddingY    10
...
//	sprintf(choiceBuf, "%s: %s", m_title, m_choices.size() ? m_choices[m_selectedChoice] : "(No Choices Available)");
	ofDrawBitmapString(m_title, KSGCBTrianglePadding * 2 + kSGCBTriangleWidth , 15);
...
//			   width - (kSGCBTriangleWidth/2 + KSGCBTrianglePadding), kSGCBTextPaddingY);
    ...
    ofTriangle(KSGCBTrianglePadding, kSGCBTextPaddingY - 4,
			   KSGCBTrianglePadding + kSGCBTriangleWidth, kSGCBTextPaddingY - 4,
			   KSGCBTrianglePadding + kSGCBTriangleWidth/2, kSGCBTextPaddingY + 5);
